...
typedef struct {
	...
	t_jrgba a_presamplebgcolor;
	t_jrgba a_lookaheadbgcolor;
	...
	//History buffers used by the presample feature
	double* a_hist1_left;
	double* a_hist1_right;
	double* a_hist2_left;
	double* a_hist2_right;

	//History indexer
	int a_hist_rec_index;
	...
	int a_pending_presamples;

	//Presamples buffer before the trigger and lookahead is used purely as a visual indicator
	int a_presamples;
	int a_lookaheadsamples;
	...
	double a_peak_left_s;
	double a_peak_right_s;
} t_stackscope;
...
void ext_main(void* r) {
	...
	CLASS_ATTR_STYLE_LABEL(c, "sig1color", 0, "rgba", "Signal1 Color");
	CLASS_ATTR_RGBA(c, "sigsumcolor", 0, t_stackscope, a_sigsumcolor);
	CLASS_ATTR_STYLE_LABEL(c, "sigsumcolor", 0, "rgba", "Signal Sum Color");
	CLASS_ATTR_RGBA(c, "aboveceilcolor", 0, t_stackscope, a_aboveceilcolor);
	CLASS_ATTR_STYLE_LABEL(c, "aboveceilcolor", 0, "rgba", "Above Ceiling Color");
...
void snap_completed(t_stackscope *x, t_symbol *s, long arc, t_atom *argv) {
	//These outlet writes are deferred to low priority from the DSP thread
	outlet_float(x->a_outlet2_peak_left, atodb(x->a_peak_left_s));
	outlet_float(x->a_outlet3_peak_right, atodb(x->a_peak_right_s));
...
void stackscope_perform64(t_stackscope* x, t_object* dsp64, double** ins, long numins, double** outs, long numouts, long sampleframes,
	...
	while (search_idx < sampleframes) {
		if (x->a_is_sampling) {
			...
			if (new_index >= x->a_samples) {
				...
				x->a_peak_left_s = peak_left_s;
				x->a_peak_right_s = peak_right_s;
				defer((t_object *)x, (method)snap_completed, NULL, 0, NULL);
			}
			else {
		...
		if (!x->a_is_sampling) {
			while (search_idx < sampleframes) {
				if (trigger[search_idx] > 0) {
					//History will always be a multiple of the vector size
					int hist_max = (HIST_SIZE / sampleframes)*sampleframes;
					...
					if (samples > 0 && (x->a_samples != samples || x->a_presamples != presamples || x->a_sig1_left == NULL))
					{
						...
						size_t newsize = (x->a_samples+x->a_presamples)*sizeof(double);
						x->a_sig1_left = (double*)sysmem_newptr(newsize);
						x->a_sig1_right = (double*)sysmem_newptr(newsize);
						x->a_sig2_left = (double*)sysmem_newptr(newsize);
						x->a_sig2_right = (double*)sysmem_newptr(newsize);
					}

					//Populate presamples
					if (presamples > 0) {
						int hi = x->a_hist_rec_index;
						
						if (hi >= presamples) {
							size_t psize = presamples*sizeof(double);
							memcpy(x->a_sig1_left, &x->a_hist1_left[hi - presamples], psize);
							memcpy(x->a_sig1_right, &x->a_hist1_right[hi - presamples], psize);
							memcpy(x->a_sig2_left, &x->a_hist2_left[hi - presamples], psize);
							memcpy(x->a_sig2_right, &x->a_hist2_right[hi - presamples], psize);
						}
						else {
							int n1 = (presamples - hi);
							int i1 = hist_max - n1;
							size_t psize1 = n1*sizeof(double);
							memcpy(x->a_sig1_left, &x->a_hist1_left[i1], psize1);
							memcpy(x->a_sig1_right, &x->a_hist1_right[i1], psize1);
							memcpy(x->a_sig2_left, &x->a_hist2_left[i1], psize1);
							memcpy(x->a_sig2_right, &x->a_hist2_right[i1], psize1);

							size_t psize2 = hi*sizeof(double);
							memcpy(&x->a_sig1_left[n1], x->a_hist1_left, psize2);
							memcpy(&x->a_sig1_right[n1], x->a_hist1_right, psize2);
							memcpy(&x->a_sig2_left[n1], x->a_hist2_left, psize2);
							memcpy(&x->a_sig2_right[n1], x->a_hist2_right, psize2);							
						}
						x->a_sig_index = x->a_presamples;
					} else {						
						x->a_sig_index = 0;
					}
	...
	//Record to history buffers for use by presamples feature
	if (x->a_hist_rec_index + sampleframes >= HIST_SIZE) {
		x->a_hist_rec_index = 0;
	}
	size_t blocksize = sampleframes*sizeof(double);
	int index = x->a_hist_rec_index;
	memcpy(&x->a_hist1_left[index], sig1_l, blocksize);
	memcpy(&x->a_hist1_right[index], sig1_r, blocksize);
	memcpy(&x->a_hist2_left[index], sig2_l, blocksize);
	memcpy(&x->a_hist2_right[index], sig2_r, blocksize);
	x->a_hist_rec_index += sampleframes;
}
...
void draw_column_with_ceil(t_stackscope* scope, t_uint8* buf, int x, int stride, double v0, double v1, t_jrgba c) {
	int v0above = fabs(v0) > scope->a_ceiling_s;
	int v1above = fabs(v1) > scope->a_ceiling_s;
	if (v0above && v1above) {
		double c0 = copysign(scope->a_ceiling_s, v0);
		double c1 = copysign(scope->a_ceiling_s, v1);
		if (c0 != c1) {			
			draw_column(scope, buf, x, stride, c0, c1, c);
			draw_column(scope, buf, x, stride, v0, c0, scope->a_aboveceilcolor);		
			draw_column(scope, buf, x, stride, c1, v1, scope->a_aboveceilcolor);
		}
		else {
		...
		double c0 = copysign(scope->a_ceiling_s, v0);
		draw_column(scope, buf, x, stride, v1, c0, c);
		draw_column(scope, buf, x, stride, v0, c0, scope->a_aboveceilcolor);		
	}
	else if (v1above) {
		double c1 = copysign(scope->a_ceiling_s, v1);
		draw_column(scope, buf, x, stride, v0, c1, c);
		draw_column(scope, buf, x, stride, v1, c1, scope->a_aboveceilcolor);
	}
	else {
...
void stackscope_assist(t_stackscope* x, void* b, long m, long a, char* s) {
	if (m == ASSIST_INLET) {
		...
		switch (a) {
			case 0:
				strncpy_zero(s, "Snapped", 512);
				break;
			case 1:
				strncpy_zero(s, "Peak Left (dB)", 512);
				break;
			case 2:
				strncpy_zero(s, "Peak Right (dB)", 512);
				break;
		}
	}
}
...
void* stackscope_new(t_symbol* s, long argc, t_atom* argv) {
	...
	if ((x = (t_stackscope*)object_alloc(stackscope_class))) {
		...
		x->a_sizey = 0;
		x->a_faulted = false;
		x->a_hist1_left = (double*)sysmem_newptrclear(HIST_SIZE*sizeof(double));
		x->a_hist1_right = (double*)sysmem_newptrclear(HIST_SIZE*sizeof(double));
